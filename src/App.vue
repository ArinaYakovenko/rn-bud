...
<template>
  <div class="min-h-screen bg-slate-50 text-slate-900 selection:bg-[#0a3357] selection:text-white">
    ...
    <header class="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
      <div class="mx-auto flex container items-center justify-between px-4 py-3 sm:px-6 lg:px-8 xl:px-10">
        ...
        <div class="hidden md:block">
          <a :href="contact.oferteo" class="btn btn-primary text-sm" target="_blank" rel="noreferrer noopener">Wycena na
            Oferteo</a>
        </div>
        ...
        <button type="button"
          ...
          :aria-label="mobileMenuOpen ? 'Zamknij menu' : 'Otwórz menu'" @click="toggleMenu">
      ...
      <div v-if="mobileMenuOpen" class="border-t border-slate-100 bg-white md:hidden">
        <nav class="mx-auto container flex flex-col px-4 py-4 gap-1">
          <router-link v-for="link in navLinks" :key="link.label" :to="link.to"
            ...
            @click="closeMenu">{{ link.label }}</router-link>
          <a :href="contact.oferteo" target="_blank" rel="noreferrer noopener"
            class="btn btn-primary mt-3 w-full text-center" @click="closeMenu">Wycena na Oferteo</a>
        </nav>
      </div>
    </header>
        ...
        <div class="grid gap-10 py-12 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <div>
            ...
            <ul class="mt-4 space-y-3 text-sm text-slate-600">
              ...
              <li>
                <a :href="contact.oferteo" target="_blank" rel="noreferrer noopener"
                  class="inline-flex items-center gap-1 font-medium hover:opacity-80" style="color: #0a3357;">
                  Profil na Oferteo →
                </a>
              </li>
            </ul>
